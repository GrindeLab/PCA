...
2) Run PCA on pruned set of variants
3) Plot PCs
4) Calculate PCA correlation
5) Plot PCA correlation
"""
...
config["out_file"] = configdict["data_prefix"] + "_snprelate.RData"
...
config = deepcopy(configdict)
config["pca_file"] = configdict["data_prefix"] + "_snprelate.RData"
...
config = deepcopy(configdict)
config["pca_file"] = configdict["data_prefix"] + "_snprelate.RData"
config["out_file"] = configdict["data_prefix"] + "_snprelate_corr_chr .gds"
configfile = configdict["config_prefix"] + "_" + job + ".config"
TopmedPipeline.writeConfig(config, configfile)
...
job = "snprelate_corr_plots"

rscript = os.path.join(pipeline, "R", job + ".R")

config = deepcopy(configdict)
config["chromosomes"] = TopmedPipeline.parseChromosomes(chromosomes)
config["corr_file"] = configdict["data_prefix"] + "_snprelate_corr_chr .gds"
config["out_prefix"] = configdict["plots_prefix"] + "_snprelate_corr"
configfile = configdict["config_prefix"] + "_" + job + ".config"
TopmedPipeline.writeConfig(config, configfile)
