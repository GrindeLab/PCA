...
create_config <- function(excl, r2, wdw, maf){
  config <- c(gds_file = 'gds/filtered_freeze5.gds',
	      out_file = paste0('data/prune_', excl, '_', r2, '_', wdw, '_', maf, '_snprelate_load.RData'),
	      pca_file = paste0('data/prune_', excl, '_', r2, '_', wdw, '_', maf, '_snprelate.RData'))
  config.df <- data.frame(config)
  rownames(config.df) <- names(config)
  write.table(config, file = paste0('config/prune_', excl, '_', r2, '_', wdw, '_', maf, '_pca_load.config'),
