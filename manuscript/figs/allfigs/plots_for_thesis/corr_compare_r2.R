...
corr_prune02 <- getobj(paste0(corrdir, 'pca_snprelate_prune_0.2_0.5_corr.RData'))
dat_prune02 <- data.frame(t(corr_prune02$snpcorr[1:4,]))
names(dat_prune02) <- paste0('PC', 1:4, '_prune0.2')
dat_prune02$rsID <- corr_prune02$snp.id
...
corr_prune01 <- getobj(paste0(corrdir, 'pca_snprelate_prune_0.1_0.5_corr.RData'))
dat_prune01 <- data.frame(t(corr_prune01$snpcorr[1:4,]))
names(dat_prune01) <- paste0('PC', 1:4, '_prune0.1')
dat_prune01$rsID <- corr_prune01$snp.id
...
#	inner_join(dat_excl, by = 'rsID') %>%
	inner_join(dat_prune02, by = 'rsID') %>%
#	inner_join(dat_both, by = 'rsID')
	inner_join(dat_prune01, by = 'rsID')
...
dat.long <- dat.long %>%
	...
	mutate(filter = factor(unlist(lapply(strsplit(PC, '_'), function(x) x[[2]])), 
		levels = c('none', 'prune0.2', 'prune0.1'),
		labels = c('none', 'prune0.2', 'prune0.1')))
dat.long$PC <- NULL
...
#	dat <- subset(dat.long, filter %in% bins[[i]])
	...
	ggsave(paste0(plotdir, 'pc_geno_corr_compare_prune.png'), plot = p, width = 15, height = 15)
