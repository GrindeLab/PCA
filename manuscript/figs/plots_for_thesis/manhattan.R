...
for(i in 1:6){
  res <- read.table(paste0(resdir, type, '_spur_', covars[i], '_beta', beta, '_snp', rep, '.assoc.linear'), header = T)
  ...
  ggsave(file = paste0(plotdir, 'manh_', type, '_', rep, '_', covars[i], '_', beta, '.png'), plot = plots[[i]], width = 10, height = 5)
}

## combine all plots
library(gridExtra)
plot.all <- grid.arrange(grobs = plots, nrow = 2)
ggsave(file = paste0(plotdir, 'manh_', type, '_', rep, '_', beta, '.png'), plot = plot.all, width = 20, height = 10) 
